...
static const short sintable[] = {0,   24,  49,  74,  97,  120, 141, 161, 180, 197, 212,
...
static modtype modlist[] = {{"M!K!", 4},  {"M.K.", 4},  {"M&K!", 4},  {"PATT", 4},  {"NSMS", 4},
                            {"LARD", 4},  {"FEST", 4},  {"FIST", 4},  {"N.T.", 4},  {"OKTA", 8},
                            {"OCTA", 8},  {"$CHN", -1}, {"$$CH", -1}, {"$$CN", -1}, {"$$$C", -1},
                            {"FLT$", -1}, {"EXO$", -1}, {"CD$1", -1}, {"TDZ$", -1}, {"FA0$", -1},
                            {"", 0}};
...
static int getnote(unsigned short period) {
    int i;
    const short* ptr;
        ...
        if(cptr->funkoffset > 128) {
            ...
            if(cptr->sampdata && cptr->length && (cptr->replen > 1)) {
                if(((cptr->samppos) >> 11) >= (unsigned long)(cptr->replen + cptr->reppnt)) {
                    cptr->samppos =
                        ((unsigned long)(cptr->reppnt) << 11) +
                        (cptr->samppos % ((unsigned long)(cptr->replen + cptr->reppnt) << 11));
                }
...
static void worknote(note* nptr, channel* cptr, modcontext* mod) {
    ...
    if(period || sample) {
        ...
        if(period || sample) {
            if(period) {
                if((effect_op != EFFECT_TONE_PORTAMENTO) ||
                   ((effect_op == EFFECT_TONE_PORTAMENTO) && !cptr->sampdata)) {
                    // Not a Tone Partamento effect or no sound currently played :
                    if((effect_op != EFFECT_EXTENDED || effect_param_h != EFFECT_E_NOTE_DELAY) ||
                       ((effect_op == EFFECT_EXTENDED && effect_param_h == EFFECT_E_NOTE_DELAY) &&
                        !effect_param_l)) {
                    ...
                    if(effect_op == EFFECT_TONE_PORTAMENTO) enable_nxt_smp = 1;
                }
        ...
        if(period) {
            if(cptr->finetune) {
                ...
                period = period_table_ptr[getnote(period)];
            }
    ...
    if(effect_op == 0xE) mod->effects_event_counts[0x10 + effect_param_h]++;
#endif
    ...
    case EFFECT_VOLSLIDE_TONEPORTA:
        ...
        if(effect_param != 0) cptr->volumeslide = effect_param;
    ...
    case EFFECT_SET_OFFSET:
        ...
        if(!cptr->samppos) cptr->samppos = cptr->last_set_offset;
    ...
    case EFFECT_PATTERN_BREAK:
        ...
        if(mod->patternpos >= 64) mod->patternpos = 63;
...
static void workeffect(modcontext* modctx, channel* cptr) {
    doFunk(cptr);
    ...
    if(modmemory) {
        if(modctx) {
            ...
            while(modlist[i].numberofchannels && !modctx->number_of_channels) {
                ...
                j = 0;
                while(j < 4) {
                    if((modlist[i].signature[j] == modctx->song.signature[j]) ||
                       modlist[i].signature[j] == '$') {
                        if(modlist[i].signature[j] == '$') {
                            if(modctx->song.signature[j] >= '0' &&
                               modctx->song.signature[j] <= '9') {
                                modctx->number_of_channels +=
                                    (modctx->song.signature[j] - '0') * digitfactor;
                                digitfactor /= 10;
                            } else {
                                modctx->number_of_channels = 0;
                                break;
            ...
            modctx->patternticksem =
                (muint)(((mulong)modctx->playrate * 5) / ((muint)modctx->bpm * 2));
#else
            // song.speed = 1 <> 31
            // playrate = 8000 <> 96000
            // bpm = 32 <> 255
            ...
            modctx->sampleticksconst = ((3546894UL * 16) / modctx->playrate)
                                       << 6; //8448*428/playrate;
    ...
    if(modctx && outbuffer) {
        if(modctx->mod_loaded) {
            ...
            if(trkbuf) {
                ...
                for(i = 0; i < 31; i++) {
                    memcopy(
                        trkbuf->instruments[i].name,
                        modctx->song.samples[i].name,
            ...
            for(i = 0; i < nbsample; i++) {
                ...
                if(modctx->patternticks++ > modctx->patternticksaim) {
                    if(!modctx->patterndelay) {
                        ...
                        for(c = 0; c < modctx->number_of_channels; c++) {
                            worknote((note*)(nptr), (channel*)(cptr), modctx);
                        ...
                        if(modctx->patternpos == 64 * modctx->number_of_channels) {
                            modctx->tablepos++;
                            modctx->patternpos = 0;
                            if(modctx->tablepos >= modctx->song.length) modctx->tablepos = 0;
                        }
                    } else {
                        modctx->patterndelay--;
                        modctx->patternticks = 0;
                ...
                if(trkbuf && !state_remaining_steps) {
                    if(trkbuf->nb_of_state < trkbuf->nb_max_of_state) {
                        memclear(
                            &trkbuf->track_state_buf[trkbuf->nb_of_state],
                            0,
                            sizeof(tracker_state));
                    }
                }
#endif
                    ...
                    if(cptr->period != 0) {
                        ...
                        if(cptr->replen < 2) {
                            if((cptr->samppos >> 11) >= cptr->length) {
                                ...
                                if(cptr->length)
                                    cptr->samppos =
                                        cptr->samppos % (((unsigned long)cptr->length) << 11);
                                else
                                    cptr->samppos = 0;
                            }
                        } else {
                               ...
                               (unsigned long)(cptr->replen + cptr->reppnt)) {
                                ...
                                if(cptr->sampdata) {
                                    cptr->samppos =
                                        ((unsigned long)(cptr->reppnt) << 11) +
                                        (cptr->samppos %
                                         ((unsigned long)(cptr->replen + cptr->reppnt) << 11));
                                }
                            }
                        }
                        ...
                        if(trkbuf && !state_remaining_steps) {
                            if(trkbuf->nb_of_state < trkbuf->nb_max_of_state) {
                                trkbuf->track_state_buf[trkbuf->nb_of_state].number_of_tracks =
                                    modctx->number_of_channels;
                                trkbuf->track_state_buf[trkbuf->nb_of_state].buf_index = i;
                                trkbuf->track_state_buf[trkbuf->nb_of_state].cur_pattern =
                                    modctx->song.patterntable[modctx->tablepos];
                                trkbuf->track_state_buf[trkbuf->nb_of_state].cur_pattern_pos =
                                    modctx->patternpos / modctx->number_of_channels;
                                trkbuf->track_state_buf[trkbuf->nb_of_state].cur_pattern_table_pos =
                                    modctx->tablepos;
                                trkbuf->track_state_buf[trkbuf->nb_of_state].bpm = modctx->bpm;
                                trkbuf->track_state_buf[trkbuf->nb_of_state].speed =
                                    modctx->song.speed;
                                trkbuf->track_state_buf[trkbuf->nb_of_state].tracks[j].cur_effect =
                                    cptr->effect_code;
                                trkbuf->track_state_buf[trkbuf->nb_of_state]
                                    .tracks[j]
                                    .cur_parameffect = cptr->parameffect;
                                if(cptr->sampinc)
                                    trkbuf->track_state_buf[trkbuf->nb_of_state]
                                        .tracks[j]
                                        .cur_period =
                                        (muint)(modctx->sampleticksconst / cptr->sampinc);
                                else
                                    trkbuf->track_state_buf[trkbuf->nb_of_state]
                                        .tracks[j]
                                        .cur_period = 0;
                                trkbuf->track_state_buf[trkbuf->nb_of_state].tracks[j].cur_volume =
                                    cptr->volume;
                                trkbuf->track_state_buf[trkbuf->nb_of_state]
                                    .tracks[j]
                                    .instrument_number = (unsigned char)cptr->sampnum;
                            }
                        }
#endif
                    }
            ...
            if(trkbuf) {
                ...
                trkbuf->name[0] = 0;
                memclear(
                    trkbuf->track_state_buf, 0, sizeof(tracker_state) * trkbuf->nb_max_of_state);
